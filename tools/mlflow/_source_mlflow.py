"""Headless source: permission-denial events from MLflow artifacts.

mcp-coder runs Claude Code headless with a restricted MCP tool set and no human,
so every call to a non-allow-listed tool is auto-denied and recorded as a
``permission_denials`` entry in the run's result message. This reader harvests
one event per denial. Native ``Bash`` is never exposed to these runs.
"""
