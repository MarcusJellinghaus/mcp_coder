...
def get_latest_runs(
    conn: sqlite3.Connection, limit: int = 5, experiment_name: Optional[str] = None
) -> List[Dict[str, Any]]:
...
def get_run_params(conn: sqlite3.Connection, run_id: str) -> Dict[str, str]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT key, value FROM params WHERE run_uuid = ? ORDER BY key", (run_id,)
    )
...
def get_run_metrics(conn: sqlite3.Connection, run_id: str) -> Dict[str, float]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT key, value FROM metrics WHERE run_uuid = ? ORDER BY key", (run_id,)
    )
...
def get_conversation_data(artifact_uri: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        conversation_file = (
            Path(artifact_path_str) / "conversation_data" / "conversation.json"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--limit", type=int, default=5, help="Number of runs to display (default: 5)"
    )
