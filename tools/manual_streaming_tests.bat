@echo off
REM Manual streaming tests for issue #603
REM Usage: tools\manual_streaming_tests.bat [ndjson|text]

REM Deactivate any active venv, then activate project venv
call deactivate 2>nul
call .venv\Scripts\activate.bat

set MCPCODER=mcp-coder
set MCP_CONFIG=.mcp.json
set FMT=%~1
if "%FMT%"=="" set FMT=ndjson
REM Valid formats: text, json, session-id, ndjson, json-raw

echo === Format: %FMT% ===
echo.

echo === Test 1: Claude CLI (no MCP) ===
%MCPCODER% prompt --llm-method claude --output-format %FMT% --timeout 60 "Count from 1 to 5. Just the numbers, one per line."
echo.

echo === Test 2: Claude CLI + MCP + sleep ===
%MCPCODER% prompt --llm-method claude --output-format %FMT% --timeout 120 --mcp-config %MCP_CONFIG% "Count from 1 to 5. Between each number, use the sleep tool to wait 3 seconds. Print each number, then sleep, then next number."
echo.

echo === Test 3: LangChain text (no MCP) ===
%MCPCODER% prompt --llm-method langchain --output-format %FMT% --timeout 60 "Count from 1 to 5. Just the numbers, one per line."
echo.

echo === Test 4: LangChain agent + MCP tools ===
%MCPCODER% prompt --llm-method langchain --output-format %FMT% --timeout 120 --mcp-config %MCP_CONFIG% "List all MCP tools available to you, one per line."
echo.

echo === Test 5: LangChain agent + file lifecycle ===
%MCPCODER% prompt --llm-method langchain --output-format %FMT% --timeout 180 --mcp-config %MCP_CONFIG% "Using tools: 1) Create _test_603.txt with 'hello'. 2) Read it. 3) Edit it to 'hello streaming'. 4) Read it. 5) Delete it. 6) List dir to confirm gone."
echo.

echo === Test 6: LangChain agent + MCP + sleep ===
%MCPCODER% prompt --llm-method langchain --output-format %FMT% --timeout 120 --mcp-config %MCP_CONFIG% "Count from 1 to 5. Between each number, use the sleep tool to wait 3 seconds. Print each number, then sleep, then next number."
echo.

echo === Done ===
