# Task Status Tracker

## Issue: #357 - Add architectural constraint - subprocess should only get started from subprocess wrapper

## Instructions for LLM

Read `pr_info/steps/summary.md` for architectural context before implementing any step.
Each step file contains detailed WHERE/WHAT/HOW specifications.

---

## Tasks

### Step 1: Re-export Exceptions and Migrate Production Code
- [x] Task 1.1: Add exception re-exports to `subprocess_runner.py`
- [x] Task 1.2: Migrate `commits.py` to use `execute_command()`
- [x] Task 1.3: Remove fallback subprocess calls from `claude_executable_finder.py`
- [x] Task 1.4: Change exception imports in `task_processing.py`
- [x] Task 1.5: Change exception imports in `claude_code_api.py`
- [x] Task 1.6: Change exception imports in `claude_code_cli.py`
- [x] Task 1.7: Change exception imports in `black_formatter.py`
- [x] Task 1.8: Change exception imports in `isort_formatter.py`
- [x] Run tests for affected modules

### Step 2: Migrate Test Files and Add Import-Linter Contract
- [x] Task 2.1: Migrate `test_issue_manager_label_update.py` to use `execute_command()`
- [x] Task 2.2: Remove unused subprocess import from `test_create_pr_integration.py`
- [x] Task 2.2b: Remove unused subprocess import from `test_main.py`
- [x] Task 2.3: Delete `test_subprocess_encoding_directly` test
- [x] Task 2.4: Change exception imports in `test_claude_code_api.py`
- [x] Task 2.5: Change exception imports in `test_claude_code_api_error_handling.py`
- [x] Task 2.6: Change exception imports in `test_claude_code_cli.py`
- [x] Task 2.7: Add subprocess isolation contract to `.importlinter`
- [x] Run `lint-imports` to verify contract
- [x] Run full test suite
