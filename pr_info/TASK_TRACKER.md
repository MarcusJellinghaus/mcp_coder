...
[ ] **Step 1.1**: Write tests for `needs_rebase()` function in `tests/utils/git_operations/test_branches.py`
...
[ ] **Step 2.1**: Write tests for `BranchStatusReport` dataclass in `tests/utils/test_branch_status.py`
[ ] **Step 2.2**: Implement `BranchStatusReport` and helpers in `src/mcp_coder/utils/branch_status.py`
...
[ ] **Step 3.1**: Write tests for `collect_branch_status()` function in `tests/utils/test_branch_status.py`
[ ] **Step 3.2**: Implement `collect_branch_status()` and helpers in `src/mcp_coder/utils/branch_status.py`
...
[ ] **Step 4.1**: Write tests for CLI command in `tests/cli/commands/test_check_branch_status.py`
...
[ ] **Step 5.1**: Write tests for CLI parser integration in `tests/cli/test_main.py`
...
## Pull Request
[ ] **PR.1**: Review all implementation steps are complete
[ ] **PR.2**: Run final quality checks across entire codebase
[ ] **PR.3**: Prepare comprehensive PR description summarizing the branch status check feature