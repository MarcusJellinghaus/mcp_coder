...
- [x] Implementation: add `run_format_code` shim to `mcp_tools_py.py`, swap import in `task_processing.py`, update tests, add `tach.toml` dependency
- [x] Quality checks: pylint, pytest, mypy — fix all issues
- [x] Commit message prepared: `refactor: add run_format_code shim + swap caller`

### Step 2: Delete formatters package and update configs
- [x] Implementation: delete `src/mcp_coder/formatters/` (6 files) and `tests/formatters/` (9 files), update `.importlinter`, `tach.toml`, `tools/tach_docs.py`, `docs/architecture/architecture.md`, `pyproject.toml`
- [x] Quality checks: pylint, pytest, mypy — fix all issues
