...
| `test_persist_write_failure_degrades_to_a_session_grant` | **parametrised** over an unwritable target (create `.icoder/settings.local.json` as a **directory**), a malformed file (`{"allow": [`), a non-object root (`["x"]`), a non-list section value (`{"allow": "mcp__a__b"}`) and a duplicate top-level key whose item cannot be located (`{"ask": [], "ask": ["mcp__srv__do_it"]}`, which drives the move branch); in all five, choice `3` still calls `add_runtime_rule`, still calls `resolve_pending`, and the output log carries a message. The last four are what prove `PersistError` reaches the `except OSError` branch rather than escaping onto the UI thread as a `ValueError`/`TypeError` and wedging the turn |
