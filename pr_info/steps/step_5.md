...
- `src/mcp_coder/cli/commands/icoder.py` — pass `permission_degraded=config.degraded` to `AppCore`
...
- **`cli/commands/icoder.py`:** in the langchain branch, pass `permission_degraded=config.degraded`
  (the `config` loaded for the gateway); non-langchain defaults to `False`.
