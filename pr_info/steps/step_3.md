...
| `tests/cli/test_main.py` | Delete `:448-513` — start at the `class TestExecutionDirArgument:` header (`:448`), **not** its docstring (`:449`), which would leave an empty class body. Delete `:683-692`. **Update, do not delete,** `test_check_branch_status_with_all_flags` (`:694-721`): it also covers `--project-dir`, `--fix`, `--llm-truncate`, `--llm-method`, `--mcp-config`. **Also drop the lone `assert args.execution_dir is None` at `:631`**, in `test_check_branch_status_default_args` (`:619`): it sits outside all three ranges above and becomes an `AttributeError` once the parser stops adding the attribute. The other six default assertions stay — same shape as the `test_cli_icoder_parser.py:54` case below. |
...
| `tests/cli/commands/test_commit.py` | **Not covered by the mechanical row below — `TestCommitAutoExecutionDir` (`:786-980`) is five tests built on the flag.** Delete `test_invalid_execution_dir_returns_error` (`:868-887`): it passes `execution_dir="/nonexistent/invalid/path"` and asserts `result == 1` **and** `"execution directory" in caplog.text.lower()`; this step deletes `commit.py:76-83` and `resolve_claude_cwd` cannot raise, so execution falls through to the unpatched real `generate_commit_message_with_llm` (only `validate_git_repository` is mocked here) and both halves fail. Delete `test_explicit_execution_dir_validated` (`:830-865`), which pins the flag being validated and used. Rename the class (e.g. `TestCommitAutoClaudeCwd`) and drop the `execution_dir=` `Namespace` lines from the surviving three (`:793`, `:894`, `:941`). **`:978-979` stays green here** — `commit.py:95` still passes `execution_dir=str(project_dir)` after this step — but it is **4b's** problem: see the `generate_commit_message_with_llm` paragraph in [step_4b.md](./step_4b.md) section B. |
| `tests/cli/commands/test_review.py` | Beyond the patched-resolver row below: drop `assert hasattr(args, "execution_dir")` (`:265`) from `test_shared_args_present` (`:257`) — it returns `False` once the parser stops adding the argument — and the "exec" from that test's docstring (`:258`). |
| `tests/integration/test_execution_dir_integration.py` | `git mv` → `tests/integration/test_claude_cwd_integration.py`. Keep **only** `test_prompt_command_no_project_dir_falls_back_to_cwd` (`:289`) and `test_prompt_command_defaults_cwd_to_project_dir` (`:345`), both in `TestSubprocessCwdParameter` (`:229`). They are the only tests patching `stream_subprocess`, i.e. spanning every layer this change rewrites. Remove the `execution_dir` marker applications (`:48`, `:150`, `:227`). **Then rewrite the surviving prose onto the new vocabulary** — the file is named `claude_cwd` after this step, so flag language left inside it is stale, and 4b's `grep` is `src/`-only, so nothing else catches it: the module docstring (`:8`, `:10`), the class docstring (`:232-233`, "ensuring `execution_dir` is used as cwd"), the retained test's docstring (`:295-301`, "No `--execution-dir` *and* no `--project-dir`") and the comments at `:314`, `:371` and especially **`:338-340`**, which describes `resolve_execution_dir`'s removed no-anchor branch by name. (The `execution_dir=None` `Namespace` attributes at `:319` and `:376` are harmless here and go with rule 2 in 4b.) |
...
> that document the removed flag, plus the inline comments that narrate the removed
> resolve-and-validate step (`commit.py:74-75`, `prompt.py:68-70`, `commit.py:98`) — see the
> docstring row in WHERE; the flag stops existing in this step, so its documentation goes with it.
>
...
> "flag is rejected" test shown in the step. In the renamed file, also rewrite the surviving
