...
| 25 | Verify smoke test error categorization | Add error categorization in Step 5's end-to-end smoke test. Catch specific exception types (`FileNotFoundError`, `ConnectionError`, `ImportError`, etc.) and provide targeted error messages instead of generic "agent test failed". Still uses `ask_llm()` as planned (Decision 6). | Better diagnostics — users can tell if the issue is config, MCP server, LLM, or dependencies. |
| 26 | Step 3 splitting | Split Step 3 into 3a (backend refactor + `_create_chat_model`) and 3b (agent routing + session + MLflow sub-commit). | Step 3 touches 6 files; splitting keeps each commit focused and reviewable. |
| 27 | Session type hints | Widen `store_langchain_history` / `load_langchain_history` signatures from `list[dict[str, str]]` to `list[dict[str, Any]]` in Step 3b. | Agent mode stores nested dicts (tool_calls, etc.); current type hints would fail mypy. |
| 28 | `overall_ok` in verify | MCP adapter package failures (`mcp_adapters`, `langgraph`) DO fail `overall_ok` in `verify_langchain()`. Update `overall_ok` computation in Step 5. | Langchain extras group always includes these packages; stricter verification. |
| 29 | Pytest marker for MCP tests | Reuse existing `langchain_integration` marker for any future MCP agent integration tests. No new marker needed. | Simpler; no additional complexity. |
| 30 | Dead transport branch in `_load_mcp_server_config` | Remove the `elif key == "transport"` branch — it's dead code since transport is always overwritten to `"stdio"` on line 139. | Dead code removal; the branch has no effect. |
| 31 | `_create_chat_model` return type | Use `TYPE_CHECKING` guard with `BaseChatModel` return type (`from __future__ import annotations` + `if TYPE_CHECKING` import). | Proper typing without breaking deferred import pattern. |
