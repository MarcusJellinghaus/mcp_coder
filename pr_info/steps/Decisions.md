...
1. **Accept loss of the INFO log `'Source label ... not present'`** in `update_workflow_label` after delegation to `transition_issue_label`. The primitive is config-free and cannot distinguish source vs clear-set labels. One test asserts this log (`test_update_workflow_label_removes_different_workflow_label`, line 635); its assertion will be dropped — behavioural assertions on the same test already cover the contract. Captured in `step_3.md` ("Delete from the old body") and in `summary.md` ("Intentional narrow behavior improvement (accepted)").
...
4. **Name the mocking style for the Step 1 token-forwarding test:** `Mock(spec=Path)` + patch `mcp_coder.utils.github_operations.base_manager.git_operations.is_git_repository` returning `True`, matching `TestBaseGitHubManagerWithProjectDir` in `test_base_manager.py`. Primary assertion: patch `user_config.get_config_values` and assert NOT called when `github_token` is explicit; IS called when `github_token=None`.
