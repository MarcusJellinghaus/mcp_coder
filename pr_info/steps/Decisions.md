...
* `step_4.md`'s mock-class rule was mis-scoped in two ways (found in the fourth review
  round, 2026-08-18): it was worded as applying to mocks "whose `astream_events` returns
  `async_events(...)`", and it listed tests "1 and 3–8" as inheriting the rule via
  `_patch_run_agent_stream`. Test 6 (`test_error_persists_nothing`) fits neither: it needs
  `astream_events` to **raise**, and `_patch_run_agent_stream` hard-wires
  `astream_events.return_value` to an events list, so it cannot express that. The rule is
  now scoped to *any* react-agent mock `run_agent_stream` consumes, regardless of return
  value, and the step lists the three groups explicitly — inherit (1, 3, 4, 5, 7, 8),
  apply explicitly (6, 11), exempt (2, 9, 10, 12) — with test 6 pointed at
  `test_error_propagation` / `_RaisingAsyncIter`
  (`test_langchain_agent_streaming.py:307-330`) as its reference shape. Entry 9 above was
  updated to match. Non-blocking (an `AsyncMock()` there fails loudly and immediately), but
  fixed on the cost-asymmetry argument: one sentence now versus rediscovering the reference
  shape mid-step.
