...
| File | Change |
|------|--------|
| `src/mcp_coder/cli/utils.py` | Add `_get_status_symbols()` |
| `src/mcp_coder/cli/parsers.py` | Add `add_verify_parser()` with `--check-models` |
...
1. **Step 1** — Extract `_get_status_symbols()` to `cli/utils.py`, add verify parser to `parsers.py`
2. **Step 2** — Refactor Claude CLI verification to return structured dict
...
6. **Step 6** — Integration tests for exit code logic and end-to-end flow
