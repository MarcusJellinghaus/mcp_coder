...
- **D3** — `langchain-mcp-adapters` floor raised `>=0.1.0` → `>=0.3.0`; a runtime capability check
  gives a clear error if `tool_interceptors` is unsupported.
...
4. **The runtime capability check folds into the existing `_check_agent_dependencies()`** rather than a
   new probe.
...
- `src/mcp_coder/llm/providers/langchain/agent.py` — capability check in `_check_agent_dependencies`;
  new `_convert_server_tools` helper; rewire `run_agent` and `run_agent_stream` onto it.
...
- `src/mcp_coder/icoder/services/llm_service.py` — `gateway` param on `RealLLMService`; replace the
  I1.1 filter block with the gateway turn filter; drop the `filter_tools_by_declaration` import and the
  `permission_warning` emission.
- `src/mcp_coder/cli/commands/icoder.py` — `load_permission_config(project_dir)`; construct the gateway;
  inject `tool_interceptors=[gate.interceptor]` into `MCPManager`; pass `gateway` to `RealLLMService`.
...
- `tests/icoder/test_llm_service.py` — updated for the gateway path / removed `permission_warning` assertions.
...
5. **Startup wiring + guards** — `load_permission_config` + gateway construction + interceptor injection
   in `icoder.py`; bypass-guard test; real-agent integration test.
...
- AC D1 "single injection point + bypass guard" → Steps 2 (helper), 5 (guard).
- AC "same canonical identity turn vs call; two servers same bare name" → Step 3.
- AC "`_cached_tools` never mutated" → Step 4.
- AC "skill-elevated `never` stays callable (synthetic frame)" → Step 3.
- AC "per-turn holder scopes frame+config" → Steps 3, 4.
- AC "adapter floor `>=0.3.0` + runtime error if unsupported" → Step 1.
