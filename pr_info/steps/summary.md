...
Inside those functions, `is_session_active(session)` calls become `active_set[session["folder"]]` lookups.
...
after `restart_closed_sessions` returns, then increments `current_count += len(started)` after each per-repo `process_eligible_issues` call.
...
### 7. Single OUTPUT framing log line

`build_active_session_set` emits `Checking N session(s)...` before the snapshot loop, so the N `is_session_active` INFO lines that follow read as one logical block instead of repeated noise.
...
| `src/mcp_coder/workflows/vscodeclaude/session_launch.py` | `process_eligible_issues` accepts `current_count`. Remove `get_active_session_count` import and call. |
...
| `tests/workflows/vscodeclaude/test_status_display.py` | Same. |
| `tests/workflows/vscodeclaude/test_closed_issues_integration.py` | Same. |
| `tests/workflows/vscodeclaude/test_sessions.py` | Replace `test_get_active_session_count` (line 277) with `test_build_active_session_set` (added in step 1); drop the old test in step 5. |

### Tests added

- New invariant test asserting `is_session_active.call_count == N_sessions` for one `launch` and one `status` run with mocked sessions. Location: extend an existing test module under `tests/cli/commands/` if `execute_coordinator_vscodeclaude` is already covered there; otherwise add `tests/workflows/vscodeclaude/test_active_set_invariant.py`. The step 5 author chooses based on what exists.

## Steps

1. **Step 1** — Add `build_active_session_set` helper; thread `active_set` into `cleanup_stale_sessions` / `get_stale_sessions`; build snapshot at top of `execute_coordinator_vscodeclaude`.
2. **Step 2** — Thread `active_set` into `restart_closed_sessions`; remove its cache-clear and in-loop PID-refresh.
3. **Step 3** — Thread `current_count` into `process_eligible_issues`; remove `get_active_session_count` call.
4. **Step 4** — Thread `active_set` into `display_status_table`; build snapshot at top of `execute_coordinator_vscodeclaude_status`; remove cache-clear from `display_status_table`.
5. **Step 5** — Remove `get_active_session_count` entirely; add invariant test for full launch/status flows.
