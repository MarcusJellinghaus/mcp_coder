...
class ApprovalBridge(Protocol):
    ...
    def detach(self) -> None:
        """Unbind the sink and clear the registry (stale-`q` guard)."""
...
* **No lock, no deque, no counter** (summary §2.2). Document *why* in the class docstring:
  every mutation runs on the single-threaded agent loop; `resolve_pending` / `cancel_all` only
  `call_soon_threadsafe`; `pending()` is a `len()` read, atomic under the GIL.
...
* `pending_ids()` returns arrival order (insertion precedes any `await`), so FIFO assertions are
  non-circular.
* State lifecycle: `attach()` clears `_pending`, `_loop`, **and `_cancelled`**; `detach()` clears
  `_pending`, `_emit`, `_loop` but **keeps `_cancelled`** — `AppCore.stream_llm` reads it *after*
...
7. Unattached engine → `request_approval` returns a deny immediately and never awaits.
8. `ApprovalDecision("deny", "session")` raises `ValueError`.
9. Static conformance: `bridge: ApprovalBridge = ApprovalEngine()`.
...
> `tests/icoder/test_permissions_approval.py` first, covering all nine cases listed in the step.
>
> The engine holds **one insertion-ordered dict** — no `asyncio.Lock`, no `deque`, no separate
> counter. Follow the pseudocode in the step exactly. Document in the class docstring: why no lock
> is needed, why the loop handle comes from `get_running_loop()` inside the coroutine, why the
> direct cancel channel exists, why `detach()` must clear the registry, and that this preserves
> R3's guarantee while dropping its lock mechanism.
>
