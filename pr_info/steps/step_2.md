...
This step lands **after** Step 1 — it imports `get_bin_dir` from `utils.mcp_verification`, which Step 1 creates.
...
Import the shared `get_bin_dir` helper from `utils.mcp_verification` (created in Step 1) and include `MCP_CODER_VENV_PATH` in the returned dict.
...
- Import `get_bin_dir` from `mcp_coder.utils.mcp_verification` (single source of truth, created in Step 1). Do NOT define a local `_get_bin_dir` duplicate.
...
**Test isolation:** Use `monkeypatch.setenv` / `monkeypatch.delenv` for `VIRTUAL_ENV` / `CONDA_PREFIX` — never mutate `os.environ` directly (pytest-xdist `-n auto` requires isolation).
