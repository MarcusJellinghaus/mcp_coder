...
**Commit message:** `feat: add watchdog set-status to coordinator templates for silent-death recovery (#713)`
...
9. **`test_templates_placeholders_still_resolve`** — verify `.format(log_level="info", issue_number="42", branch_name="42-feat")` succeeds without `KeyError`
