...
- `src/mcp_coder/llm/formatting/stream_renderer.py` — only if a small reusable "render tool block compressed" helper is needed (likely not; existing `format_tool_start` + body builder already cover it)
...
- `_render_unit_atomic(unit)`:
  - `unit.kind == "tool"`:
    - if `effective_tier(unit.id) == "oneline"`: synthesize a `ToolStart`/`ToolResult` pair from unit fields and call `format_tool_oneline(...)`. Return `([oneline], STYLE_TOOL_OUTPUT)`.
    - else: reuse the existing compressed rendering — `format_tool_start` lines + `│  output` lines + `└ done`/`└ done (N lines, truncated to M)` line. This is currently inline in `app.py` (`_handle_stream_event` → `ToolResult` branch); extract into a helper in `output_log.py` (or in `stream_renderer.py` and re-use).
...
> - Reuse existing compressed-block formatting logic from `app.py`; extract into a shared helper in `output_log.py` so both the live path (step 9) and `rebuild()` use the same code.
