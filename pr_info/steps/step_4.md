...
No new `pyproject_config` function: Step 1's `get_install_extras` returns `str | None`, so
`get_install_extras(folder_path) is None` *is* the declared-vs-absent test this step needs.
Step 1's `_load_pyproject` stays private to its own module.
...
- Reads the extras row via `get_install_extras` and overrides via
  `get_github_install_config`, both lax (a malformed pyproject is the installer's
  problem, and it fails there with the file named — a malformed file therefore yields
  `None` here and warns like an absent one). `validate_target_repo` imports only public
  names from `pyproject_config` — no cross-package use of `_load_pyproject`.
...
validate_target_repo(folder_path):
    ...
    if get_install_extras(folder_path) is None:  warn "falling back to 'dev'"
...
- `extras = ""` counts as declared — no extras warning. Step 1 already pins
  `get_install_extras`'s `None`-vs-`""` distinction; this is the row that consumes it.

## LLM PROMPT

> Read `pr_info/steps/summary.md` and `pr_info/steps/step_4.md`.
>
> Implement Step 4 only. TDD: write
> `tests/workflows/vscodeclaude/test_validate_target_repo.py` first, then add
> `validate_target_repo` to `session_launch.py`, swap the call at `:175`, and export it
> from the package `__init__`.
>
> Add **no** new `pyproject_config` function. The extras row is
> `get_install_extras(folder_path) is None` — Step 1's reader already returns `None` for
> "not declared". `validate_target_repo` must use only public `pyproject_config` names —
> do not import `_load_pyproject` across packages.
>
...
> the fast pytest selection. Commit once, green.
