...
The shared ``env`` fixture and the ``_status``/``_resp``/``_reviewer``/``_run``/
``_label_transition`` helpers live in ``conftest``; the Gate 2 exit-guard tests
live in ``test_core_gates``.
"""
