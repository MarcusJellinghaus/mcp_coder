...
class TestVSCodeProcessNameMatching:
    ...
    ("code"), causing processes like mcp-tools-py.exe to be mistakenly
    ...
    def test_vscode_process_names_excludes_code_checker(self) -> None:
        """mcp-tools-py.exe is not in VSCODE_PROCESS_NAMES (substring trap)."""
        assert "mcp-tools-py.exe" not in VSCODE_PROCESS_NAMES

    def test_check_vscode_running_rejects_mcp_tools_py(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """check_vscode_running returns False for mcp-tools-py.exe PIDs."""
        mock_process = MagicMock()
        mock_process.name.return_value = "mcp-tools-py.exe"
    ...
    def test_get_vscode_processes_excludes_mcp_tools_py(self) -> None:
        """_get_vscode_processes does not include mcp-tools-py.exe processes."""
        # Build mock process entries: one real VSCode, one mcp-tools-py
        ...
        mock_checker.info = {
            "pid": 2002,
            "name": "mcp-tools-py.exe",
            "cmdline": [r"C:\tools\mcp-tools-py.exe", "--project", "my_repo_42"],
        }
        ...
        assert 2002 not in pids  # mcp-tools-py excluded
