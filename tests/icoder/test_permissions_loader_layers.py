"""Tests for the iCoder permission loader layer handling (Steps 4-6, TDD).
...
Step 6 covers the public ``load_permission_config`` — merging layers, degrading
fail-closed on any broken layer, preserving per-rule provenance, and emitting
the schema when ``.icoder/`` exists.
"""
