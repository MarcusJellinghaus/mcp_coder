...
class TestClaudeCodeApiIntegration:
    ...
    def test_real_api_call(self) -> None:
        ...
        try:
            ...
            print(
                f"✓ Message types imported: {[cls.__name__ for cls in [AssistantMessage, TextBlock, SystemMessage, ResultMessage]]}"
            )
        ...
        try:
            ...
            if "4" in result:
                ...
                print(
                    f"⚠ Response doesn't contain '4' but API call succeeded: {result}"
                )
            ...
            detailed_result = ask_claude_code_api_detailed_sync(
                "What is 3+3? Just the number.", timeout=60
            )

            assert isinstance(
                detailed_result, dict
            ), f"Expected dict, got {type(detailed_result)}"
            assert (
                "text" in detailed_result
            ), "Detailed result should contain 'text' key"
            assert (
                "session_info" in detailed_result
            ), "Detailed result should contain 'session_info' key"
            assert (
            ...
            print(
                f"  Duration: {detailed_result['result_info'].get('duration_ms', 0)}ms"
            )
            print(
                f"  Tools available: {len(detailed_result['session_info'].get('tools', []))}"
            )
            ...
            if e.stderr and (
                "authentication" in e.stderr.lower() or "login" in e.stderr.lower()
            ):
