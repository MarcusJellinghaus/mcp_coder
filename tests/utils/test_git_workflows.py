...
class TestGitBranchOperations:
    ...
    def test_create_branch_from_specific_branch(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
        ...
        result = create_branch(
            "feature-from-main", project_dir, from_branch=original_branch
        )
        assert result is True
    ...
    def test_create_branch_already_exists(
        ...
        repo.git.checkout(
            "main" if "main" in [h.name for h in repo.heads] else "master"
        )
    ...
    def test_create_branch_invalid_base_branch(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
        ...
        result = create_branch(
            "test-branch", project_dir, from_branch="nonexistent-branch"
        )
    ...
    def test_checkout_branch_already_on_branch(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_checkout_branch_nonexistent_branch(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_checkout_branch_invalid_name(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_checkout_branch_from_detached_head(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_push_branch_success(self, git_repo_with_files: tuple[Repo, Path]) -> None:
        ...
        # Add a fake origin remote to the repo
        try:
        ...
        # Mock GitPython's underlying command execution to avoid network operations
        with patch("git.cmd.Git.execute") as mock_execute:
            mock_execute.return_value = ""
            ...
            # Verify git push command was executed
            assert mock_execute.called
            # Check that push command was called (may be multiple execute calls)
            push_calls = [
                call
                for call in mock_execute.call_args_list
                if any("push" in str(arg) for arg in call[0])
            ]
            assert len(push_calls) > 0, "Expected at least one push command execution"
    ...
    def test_push_branch_nonexistent_branch(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_push_branch_invalid_name(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_push_branch_no_origin_remote(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_push_branch_git_command_error(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
        ...
        with patch("subprocess.Popen") as mock_popen:
            ...
            mock_process.communicate.return_value = (b"", b"fatal: remote error")
    ...
    def test_fetch_remote_custom_remote(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
        ...
        # Add a real upstream remote for testing
        try:
            repo.create_remote("upstream", "https://github.com/example/upstream.git")
        except Exception:
    ...
    def test_fetch_remote_nonexistent_remote(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_fetch_remote_invalid_name(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
    ...
    def test_fetch_remote_git_command_error(
        self, git_repo_with_files: tuple[Repo, Path]
    ) -> None:
        ...
        with patch("subprocess.Popen") as mock_popen:
            mock_process = mock_popen.return_value
            mock_process.communicate.return_value = (b"", b"fatal: network error")
            mock_process.returncode = 128
