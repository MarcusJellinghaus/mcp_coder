...
class TestJenkinsClientStartJob:
    ...
    def test_start_job_with_params(self, mock_jenkins_class: MagicMock) -> None:
        ...
        mock_client.build_job.assert_called_once_with("test-job", parameters=params)
    ...
    def test_start_job_folder_path(self, mock_jenkins_class: MagicMock) -> None:
        ...
        mock_client.build_job.assert_called_once_with(
            "folder/subfolder/job-name", parameters={}
        )
...
class TestJenkinsClientGetQueueSummary:
    ...
    @patch("mcp_coder.utils.jenkins_operations.client.Jenkins")
    def test_get_queue_summary_success(self, mock_jenkins_class: MagicMock) -> None:
        """Test successful queue summary retrieval."""
        # Setup
        mock_client = Mock()
        mock_jenkins_class.return_value = mock_client

        # Mock queue info with 2 items
        mock_client.get_queue_info.return_value = [
            {"id": 1, "why": "Waiting"},
            {"id": 2, "why": "Waiting"},
        ]

        # Mock running builds with 3 items
        mock_client.get_running_builds.return_value = [
            {"number": 1},
            {"number": 2},
            {"number": 3},
        ]
    ...
    @patch("mcp_coder.utils.jenkins_operations.client.Jenkins")
    def test_get_queue_summary_empty(self, mock_jenkins_class: MagicMock) -> None:
        """Test queue summary with no jobs."""
        # Setup
        mock_client = Mock()
        mock_jenkins_class.return_value = mock_client

        # Mock empty queue and no running builds
        mock_client.get_queue_info.return_value = []
        mock_client.get_running_builds.return_value = []
    ...
    @patch("mcp_coder.utils.jenkins_operations.client.Jenkins")
    def test_get_queue_summary_error(self, mock_jenkins_class: MagicMock) -> None:
        """Test errors are wrapped as JenkinsError."""
        # Setup
        mock_client = Mock()
        mock_jenkins_class.return_value = mock_client
        mock_client.get_queue_info.side_effect = JenkinsException("Connection timeout")

        client = JenkinsClient("http://jenkins:8080", "user", "token")

        # Execute & Verify
        with pytest.raises(JenkinsError, match="Failed to get queue summary"):
            client.get_queue_summary()
