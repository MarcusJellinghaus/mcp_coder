...
class TestJobStatus:
    ...
    def test_job_status_creation_with_none_values(self) -> None:
        """Test JobStatus creation with None for optional fields."""
        status = JobStatus(
            status="queued",
            build_number=None,
            duration_ms=None,
    ...
    def test_job_status_str_queued(self) -> None:
        """Test JobStatus __str__() for queued job."""
        status = JobStatus(
            status="queued",
            build_number=None,
            duration_ms=None,
    ...
    def test_job_status_str_running(self) -> None:
        """Test JobStatus __str__() for running job."""
        status = JobStatus(
            status="running",
            build_number=42,
            duration_ms=None,
    ...
    def test_job_status_str_completed(self) -> None:
        """Test JobStatus __str__() for completed job."""
        status = JobStatus(
            status="SUCCESS",
            build_number=42,
            duration_ms=1234,
    ...
    def test_job_status_str_completed_failure(self) -> None:
        """Test JobStatus __str__() for failed job."""
        status = JobStatus(
            status="FAILURE",
            build_number=99,
            duration_ms=5678,
    ...
    def test_job_status_str_no_build_number(self) -> None:
        """Test JobStatus __str__() when status is not queued but no build number."""
        status = JobStatus(
            status="ABORTED",
            build_number=None,
            duration_ms=None,
...
class TestQueueSummary:
    ...
    def test_queue_summary_str_empty(self) -> None:
        """Test QueueSummary __str__() with no jobs."""
        summary = QueueSummary(running=0, queued=0)

        assert str(summary) == "0 jobs running, 0 jobs queued"

    def test_queue_summary_str_multiple_queued_single_running(self) -> None:
        """Test QueueSummary __str__() with multiple queued, single running."""
        summary = QueueSummary(running=1, queued=5)

        assert str(summary) == "1 job running, 5 jobs queued"

    def test_queue_summary_str_single_queued_multiple_running(self) -> None:
        """Test QueueSummary __str__() with single queued, multiple running."""
        summary = QueueSummary(running=4, queued=1)

        assert str(summary) == "4 jobs running, 1 job queued"
