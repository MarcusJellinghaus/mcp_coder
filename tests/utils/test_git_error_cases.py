...
class TestGitErrorCases:
    ...
    def test_non_git_directory_operations(self, tmp_path: Path) -> None:
        ...
        assert commit_result["error"] is not None and "git repository" in commit_result["error"].lower()
    ...
    def test_gitignore_behavior(self, git_repo: tuple[Repo, Path]) -> None:
        ...
        result = stage_specific_files(ignored_files[:1], project_dir)  # Try one ignored file
    ...
    def test_file_deletion_handling(self, git_repo_with_files: tuple[Repo, Path]) -> None:
        ...
        assert any(deleted_filename in f for f in modified_files) or len(modified_files) > 0
