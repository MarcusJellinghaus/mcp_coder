...
def create_pull_request(
    ...
    Returns:
        Tuple of (PullRequestData dict, None) on success,
    ...
    if base_branch is None:
        ...
        return None, "Could not detect base branch for PR creation"
...
def _add_pr_assignee_best_effort(project_dir: Path, pr_number: int) -> None:
    ...
    try:
        ...
        # add_assignees is provided by the floating mcp-workspace pin; suppress
        # static-analysis complaints when the locally installed shim lags behind.
        PullRequestManager(project_dir).add_assignees(  # type: ignore[attr-defined]  # pylint: disable=no-member
            pr_number, username
        )
        ...
        if update_issue_labels:
            if cached_issue_number is None:
                ...
                try:
                    ...
                    if success:
                        logger.log(
                            OUTPUT, "✓ Issue label updated: pr-creating → pr-created"
                        )
                    else:
