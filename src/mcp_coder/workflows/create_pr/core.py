...
def parse_pr_summary(llm_response: str) -> Tuple[str, str]:
    ...
    # Strip Claude footers from body only (not title)
...
def parse_pr_summary(llm_response: str) -> Tuple[str, str]:
    ...
    # Final fallbacks
    title = title_match or "Pull Request"
    # Only use fallback if body was not found at all (not if it's just empty)
...
def run_create_pr_workflow(
    ...
    log_step("Create PR workflow completed successfully!")
    return 0
>>>>>>> 09f6743 (fix: handle empty body content in parse_pr_summary())
