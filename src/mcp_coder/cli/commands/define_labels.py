...
def execute_define_labels(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        gen_actions = getattr(args, "generate_github_actions", False) or getattr(
            args, "all", False
        )
        ...
        # Generate GitHub Action workflow files if requested
        if gen_actions:
            written = write_github_actions(project_dir, labels_config, dry_run=dry_run)
            for path in written:
                logger.info("Wrote workflow: %s", path)
