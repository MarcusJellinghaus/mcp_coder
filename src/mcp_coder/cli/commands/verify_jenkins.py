...
from ...utils.jenkins_operations.client import JenkinsClient, get_jenkins_config
...
def _configured(value: Any) -> str:
    """Normalise one credential field to "" when it is not really configured.

    Blank-but-present values are what ``JenkinsClient.__init__`` rejects with
    ValueError. Screening them here means any later construction failure is a
    genuine malformed-URL problem worth reporting, not a missing field the
    CONFIG section already covers.

    Args:
        value: Raw credential value from the config resolver.

    Returns:
        The stripped string, or "" when the field is absent or blank.
    """
    return value.strip() if isinstance(value, str) else ""


def _unusable_server_result(server_url: str, exc: Exception) -> dict[str, Any]:
    """Build the JENKINS section for a server_url no HTTP client can be built from.

    A malformed URL surfaces from deep inside requests/urllib3 and cannot be
    caught by type: a scheme-less value such as "jenkins.example.com" leaves
    ``urllib3.util.parse_url`` with no scheme and reaches
    ``Session.mount(None, ...)``, which raises ``TypeError``. Reporting it as a
    failed Server row is the whole point of this section; letting it escape
    would abort ``mcp-coder verify`` before any other check runs.

    Args:
        server_url: The configured URL, shown on the row.
        exc: Whatever construction raised.

    Returns:
        ``_format_section``-shaped dict with a failed ``server`` row.
    """
    return {
        "server": {
            "ok": False,
            "value": server_url,
            "error": (
                f"unusable server_url ({type(exc).__name__}: {exc}) - it must "
                "include the scheme, e.g. https://jenkins.example.com:8080"
            ),
            "install_hint": _INSTALL_HINT,
        },
        "authentication": dict(_NOT_CHECKED),
        "overall_read": dict(_NOT_CHECKED),
        "overall_ok": False,
    }
...
def _build_server_result(session: Any, base_url: str, username: str) -> dict[str, Any]:
    ...
    Args:
        session: python-jenkins' own requests session (see
            ``JenkinsClient.probe_session``). Annotated ``Any`` so this module need not
...
def verify_jenkins() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    Credentials come from ``get_jenkins_config()`` rather than
    ...
    Returns:
        ...
        coordinator repos name an executor job. Never raises: a configured but
        unusable server_url comes back as a failed ``server`` row.
    """
    try:
        credentials = get_jenkins_config()
    except ValueError:
        # Malformed TOML - the CONFIG section already reports it.
        logger.debug("Skipping Jenkins verification: config could not be read")
        return {}, {}

    server_url = _configured(credentials.get("server_url"))
    username = _configured(credentials.get("username"))
    api_token = _configured(credentials.get("api_token"))
    if not (server_url and username and api_token):
        return {}, {}

    try:
        client = JenkinsClient(server_url, username, api_token)
        session = client.probe_session
        base_url = client.base_url
    except Exception as exc:  # pylint: disable=broad-exception-caught
        # Credentials are present but the client will not build. The cause is a
        # malformed server_url, and it arrives as anything from ValueError to
        # TypeError depending on how far into requests/urllib3 it gets - so
        # report it rather than filtering by type and crashing on the rest.
        logger.debug("Jenkins client could not be constructed", exc_info=True)
        return _unusable_server_result(server_url, exc), {}
